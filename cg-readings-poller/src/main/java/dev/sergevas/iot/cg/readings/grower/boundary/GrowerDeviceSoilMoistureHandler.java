...
import static dev.sergevas.iot.cg.readings.event.model.MeasurementUnits.PERCENTAGE;
import static dev.sergevas.iot.cg.readings.shared.model.DeviceTypes.GROWER;
import static dev.sergevas.iot.cg.readings.shared.model.SensorTypes.SOIL_MOISTURE;
import static dev.sergevas.iot.cg.readings.shared.model.SensorTypes.SOIL_TEMP;

@ApplicationScoped
public class GrowerDeviceSoilMoistureHandler extends GrowerDeviceRequestHandler {

    public ReadingsEvent toReadingsEvent(GrowerDeviceRequest request) {
        return new ReadingsEventBuilder()
                .data(request.getSoilMoisture())
                ...
                .natsSubject(createSubjectName(request.getDeviceId(), SOIL_MOISTURE))
                .build(x -> x);
    }
}
